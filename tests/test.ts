...
describe("Test for TrelloApi", function() {
  it("Get opened boards", async function() {
    let openedBoard = await new Request(
      `${cred.baseUrl}1/Members/andreymelnichenko3?boards=open&key=${cred.key}&token=${cred.token}`
    )
      .method("GET")
      .send();
    expect(openedBoard.statusCode).to.equals(200);
    expect(openedBoard.body).to.be.jsonSchema(schema.mainOpenBoardSchema);
    console.log(JSON.stringify(openedBoard.body));
  });

  xit("Create board", async function() {
    let createBoardByName = await new Request(
      `${cred.baseUrl}1/boards/?key=${cred.key}&token=${cred.token}`
    )
      .method("POST")
      .body({ name: "TrelloIgorApi_" + time })
      .send();
  ...
  xit("Get all lists in new board and get id for done list", async function() {
    let getBoardById = await new Request(
      `${cred.baseUrl}1/boards/${newBoardId}/lists?key=${cred.key}&token=${cred.token}`
    )
      .method("GET")
      .send();
  ...
  xit("Create list in a new board", async function() {
    let createNewList = await new Request(
      `${cred.baseUrl}1/lists/?key=${cred.key}&token=${cred.token}`
    )
      .method("POST")
      .body({ name: "List " + time, idBoard: newBoardId })
      .send();
  ...
  xit("Create a card in new list", async function() {
    let createNewCard = await new Request(
      `${cred.baseUrl}1/cards/?key=${cred.key}&token=${cred.token}`
    )
      .method("POST")
      .body({ idList: newListId, name: "Card " + time })
      .send();
  ...
  xit("Edit created card", async function() {
    let editNewCard = await new Request(
      `${cred.baseUrl}1/cards/${newCardId}/?key=${cred.key}&token=${cred.token}`
    )
      .method("PUT")
      .body({ name: "Card was edited see desc", desc: "Was edited at " + time })
      .send();
  ...
  xit("Move created card to done", async function() {
    let moveToDone = await new Request(
      `${cred.baseUrl}1/cards/${newCardId}/?key=${cred.key}&token=${cred.token}`
    )
      .method("PUT")
  ...
  xit("Delete created card", async function() {
    let deleteCreatedCard = await new Request(
      `${cred.baseUrl}1/cards/${newCardId}/?key=${cred.key}&token=${cred.token}`
    )
      .method(`DELETE`)
      .send();
  ...
  xit("Delete board", async function() {
    let deleteCreatedBoard = await new Request(
      `${cred.baseUrl}1/boards/${newBoardId}/?key=${cred.key}&token=${cred.token}`
    )
      .method("DELETE")
      .send();
